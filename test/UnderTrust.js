...
contract('Signable', (accounts) => {
    it('verifies the trust and status after construction', async () => {
        let contract = await Signable.new(accounts[1], {from: accounts[8]});
    ...
    it('verifies that only a trust can initiate deactivate', async () => {
        let contract = await Signable.new(accounts[9], {from: accounts[8]});
        try {
    ...
    it('verifies that it will not duplicate signature', async () => {
        let contract = await Signable.new(accounts[1]);
